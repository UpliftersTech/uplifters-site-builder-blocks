 ...
 *
 * Uses the Uplifters Website Builder mark from
 * src/assets-shared/brand-icon/icon.svg. ARTWORK and ANIMATION_CSS are
 * generated from icon.svg and must be regenerated together with the other
 * brand-icon copies whenever it changes.
 *
 * Each mounted icon keeps one unique id prefix for its lifetime, so multiple
 * instances cannot collide inside Gutenberg, and re-renders of the inserter
 * never rewrite the SVG.
 *
...
(function (wp) {
	...
	function UpliftersSiteBuilderBlocksCategoryRegisterIcon() {
		...
		return createElement('svg', {
			...
			dangerouslySetInnerHTML: {
				__html: (ANIMATED ? '<style>' + ANIMATION_CSS + '</style>' : '') + ARTWORK.split('__ID__').join(idPrefix),
			},
		});
	}
	...
	/**
	 * Slug prefix every category this plugin registers shares.
	 *
	 * The list of categories lives in PHP
	 * (includes/blocks-route/blocks-category-register.php). Matching on the
	 * prefix instead of restating the slugs here means a category added or
	 * renamed there picks up the icon with no change to this file - an
	 * earlier hard-coded list had drifted out of sync and left every
	 * category but one unbranded.
	 */
	var CATEGORY_SLUG_PREFIX = 'uplifters-site-builder-blocks-';

	wp.domReady(function () {
		if (typeof wp.blocks.getCategories !== 'function') {
			return;
		}

		wp.blocks.getCategories().forEach(function (category) {
			if (!category || typeof category.slug !== 'string') {
				return;
			}

			if (category.slug.indexOf(CATEGORY_SLUG_PREFIX) !== 0) {
				return;
			}

			/*
			 * A non-string title is one this script already replaced, so
			 * re-running would nest the icon inside its own output.
			 */
			if (typeof category.title !== 'string') {
				return;
			}

			wp.blocks.updateCategory(category.slug, {
				title: UpliftersSiteBuilderBlocksCategoryRegisterTitle(category.title),
			});
		});
	});
})(window.wp);
