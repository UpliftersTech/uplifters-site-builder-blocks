...
/*
 * Posts Layout starts with exactly these 10 blocks, one per row, in this order
 * (see POSTS_LAYOUT_TEMPLATE in the block's edit.js). Reuse each block's own
 * dedicated inserter-preview component here instead of a second, parallel set
 * of mini demos — and instead of a scrolling list of block *names*, actually
 * show what each block looks like as the track scrolls past it, one full demo
 * at a time.
 */
const DEMOS = [
	PostsTitlePreview,
...
const VISIBLE = 1;
const TOTAL = DEMOS.length; // 10
// Repeat the first `VISIBLE` demo(s) at the end of the track so the scroll
// can wrap seamlessly: once it has shifted past all 10 unique demos, the
