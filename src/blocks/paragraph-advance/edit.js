...
[
  ...
  // These inline formats are shared with the other text blocks, so whichever
  // block script loads first registers them. Re-registering the same name logs
  // an editor error, hence the lookup before registering. The lookup has to go
  // through the rich-text store — @wordpress/rich-text exports no standalone
  // getFormatType().
  try {
    if (select(richTextStore)?.getFormatType?.(name)) return;
    registerFormatType(name, settings);
  } catch (error) { /* Already registered during hot reload. */ }
});
...
function smartDefaults(device) {
  // Desktop, tablet and mobile paragraph defaults, in DEVICES order.
  const defaults = [[18, 1.75, '400'], [17, 1.72, '400'], [16, 1.68, '400']];
  const [fontSize, lineHeight, fontWeight] = defaults[DEVICES.indexOf(device)] || defaults[0];
...
function Editor({ attributes, setAttributes, clientId }) {
  ...
  useEffect(() => {
    ...
    const className = 'uplifters-site-builder-blocks-paragraph-advance-is-selected';
  ...
  const defaults = smartDefaults(device);
  ...
  const blockProps = useBlockProps({
    className: `uplifters-site-builder-blocks-paragraph-advance-editor uplifters-site-builder-blocks-paragraph-advance-editor--${device}`,
    style,
...
export default function Edit(props) {
	return props.attributes.preview ? <InserterPreview type="paragraph-advance" /> : <Editor {...props} />;
}